...
function AccessoryProductDetail({ lang, onClose, product, productList, setProduct, t }) {
  const [activeMedia, setActiveMedia] = useState({ type: 'image', src: normalizeImagePath(product?.image) || '/oscar-cover.webp', webpSrc: product?.image_webp ? normalizeImagePath(product.image_webp) : null });
  ...
  useEffect(() => {
    ...
    setActiveMedia({ type: 'image', src: normalizeImagePath(product.image || product.images?.[0]) || '/oscar-cover.webp', webpSrc: product.image_webp ? normalizeImagePath(product.image_webp) : null });
  ...
  // Boss 2026-09-03: parallel webp URLs from API (images_webp field).
  // Empty entries mean attachment has no webp variant yet → SmartImage falls back to JPG.
  const productImagesWebp = (product.images_webp?.length === productImages.length
    ? product.images_webp.map(normalizeImagePath)
    : productImages.map(() => null)
  );
  const mediaItems = [...productImages.map((src, i) => ({ type: 'image', src, webpSrc: productImagesWebp[i] || null, label: product.name })), ...(product.video ? [{ type: 'video', src: product.video, label: `${product.name} video` }] : [])];
        ...
        <article className="product-modal detail-view pro-detail sales-detail landing-detail-card" aria-labelledby="product-modal-title">
          <div className="detail-gallery tech-gallery">
            ...
            {activeMedia.type === 'video'
              ...
              : <SmartImage src={normalizeImagePath(activeMedia.webpSrc || activeMedia.src)} srcFallback={normalizeImagePath(activeMedia.src)} alt={product.name} width={800} height={600} priority sizes="(max-width: 760px) 100vw, 800px" />}
            <div className="gallery-thumbs">
              {mediaItems.map((item, index) => (
                <button type="button" className={activeMedia.src === item.src ? 'active' : ''} key={`${item.type}-${index}`} onClick={() => { trackEvent(item.type === 'video' ? 'product_video_click' : 'product_image_click', productParams(product, { image_index: index })); setActiveMedia(item); }}>
                  {item.type === 'video' ? <span className="video-thumb">▶ Video</span> : <SmartImage src={normalizeImagePath(item.webpSrc || item.src)} srcFallback={normalizeImagePath(item.src)} alt={item.label} width={80} height={80} sizes="80px" />}
                </button>
              ))}
            </div>
          ...
          <div className="detail-scroll">
            <div className="detail-info buy-box">
              ...
              <h1 id="product-modal-title" className="product-modal-title">{product.name}</h1>
                  ...
                  {similar.map((item) => (
                    <button className="related-card" key={item.id} onClick={() => setProduct(item, 'related_product')}>
                      <span className="related-image">
                        <SmartImage src={normalizeImagePath(item.image_webp || item.image)} srcFallback={normalizeImagePath(item.image)} alt="" width={220} height={165} sizes="(max-width: 760px) 50vw, 220px" />
                        <em>{item.brand}</em>
                      </span>
...
function LaptopProductDetail({ lang, onClose, product, productList, setProduct, t, onOrderTotalChange }) {
  ...
  // Boss 2026-09-11 (PDP p1089 fix): laptop-moi has only 1 SP → same-category match returns 0.
  // Fallback to same-brand (excluding accessories) before hiding section entirely.
  let similar = productList.filter((item) => item.category === product.category && item.id !== product.id).slice(0, 4);
  if (similar.length === 0 && product.brand) {
    similar = productList.filter((item) => item.brand === product.brand && item.id !== product.id && item.category !== 'phu-kien').slice(0, 4);
  }
  ...
  // Boss 2026-09-03: parallel webp URLs from API (images_webp field).
  const productImagesWebp = (displayProduct.images_webp?.length === productImages.length
    ? displayProduct.images_webp.map(normalizeImagePath)
    : productImages.map(() => null)
  );
  const mediaItems = [...productImages.map((src, i) => ({ type: 'image', src, webpSrc: productImagesWebp[i] || null, label: product.name })), ...(displayProduct.video ? [{ type: 'video', src: displayProduct.video, label: `${product.name} video` }] : [])];
